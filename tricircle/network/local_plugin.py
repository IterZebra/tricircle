...
tricircle_opts = [
    ...
    cfg.StrOpt('tricircle_url', help=_('Central Neutron server url')),
...
SUPPORTED_FILTERS = ['id', 'name', 'region_name', 'project_id',
                     'resource_type','status','admin_state_up']
...
#
    ...
    def after_resource_update(self, resource, event, trigger,
                                       **kwargs):
        context = kwargs['context']
        _resource_arg = kwargs[resource]
        resource_arg = {}
        for field in ('name', 'status','admin_state_up'): 
            value = _resource_arg.get(field,None)
            if value is not None:
                resource_arg[field] = value
        resource_body = { 
        ...
        print "MaXiao after_resource_updated called for resource : "+str(resource)+" event: "+str(event)+"resource_body: "+str(resource_body)
        t_ctx = t_context.get_context_from_neutron_context(context)
        try:
            print "MaXiao after_resource_updated called for resource : "+str(resource)+" event: "+str(event)+"resource_arg: "+str(resource_arg)
            result = self.tricircle_handle.handle_update(
                t_ctx, 'tricircle_resource', resource_body)
            print "MaXiao after_resource_updated called for resource : "+str(resource)+" event: "+str(event)+"resource_arg: "+str(resource_arg)
            return result
        except Exception as e:
    ...
    def _delete_resource(self,resouce_id):
        try:
            result = self.tricircle_handle.handle_delete(
                t_ctx, 'tricircle_resource', resouce_id)
            return result
        except Exception as e:
            print "MaXiao exception "+str(e)
            return {}
    ...
    def after_network_delete(self, resource, event, trigger, context,network_id ):
        print "MaXiao before_resource_delete called for resource : "+str(resource)+" event: "+str(event)+"trigger: "+str(trigger)
        t_ctx = t_context.get_context_from_neutron_context(context)
        region_name = self._get_neutron_region()
        return self._delete_resource(network_id)

    def after_router_delete(self, resource, event, trigger, context,
                            router_id, original, **kwargs):
        print "MaXiao before_resource_delete called for resource : "+str(resource)+" event: "+str(event)+"trigger: "+str(trigger)
        t_ctx = t_context.get_context_from_neutron_context(context)
        region_name = self._get_neutron_region()
        return self._delete_resource(router_id)


    def after_subnet_delete(self, resource, event, trigger, context,
                               subnet, **kwargs)
        print "MaXiao before_resource_delete called for resource : "+str(resource)+" event: "+str(event)+"trigger: "+str(trigger)
        return self._delete_resource(subnet)


    def after_port_delete(self, resource, event, trigger, context,
                               port, **kwargs)
        print "MaXiao before_resource_delete called for resource : "+str(resource)+" event: "+str(event)+"trigger: "+str(trigger)
        return self._delete_resource(port)
